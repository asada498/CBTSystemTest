@inject('Q5S3Q4', 'App\Http\Controllers\Q5\Listening\Q5S3Q4Controller')
...
function init()
  ...
  $.ajax({
    headers: {
      ...
      type:'POST',
      url:'/nat-test/saveChoiceRequestQ5S3Q4',
...
<body style="font-size: 19px;" onload="init();">
        ...
        <div class = "title1">
            <img src="{{ asset('image/q5s3q4example.png') }}" alt="Image"/>
        </div>
           
        <div class="box">
          <div id="table_data">
            <form method="post" action="{{ url('/Q5ListeningQ4SubmitData')}}" name="yourForm">
              {{ csrf_field() }}
@if(!(app()->isProduction()))
...
@endif
                  <br>
                  <p>
                  <span class="sec3-q4-c1-tight"><input type="radio" name="{{$post->getNo()}}" id="{{$post->getNo()}}a" value="a" {{ $Q5S3Q4->checkValue($post->getNo(),"a") }} onclick=" myFunction(this)" /><label for="{{$post->getNo()}}a">&nbsp a.</label></span>
                  <span class="sec3-q4-c1-tight"><input type="radio" name="{{$post->getNo()}}" id="{{$post->getNo()}}b" value="b" {{ $Q5S3Q4->checkValue($post->getNo(),"b") }} onclick=" myFunction(this)" /><label for="{{$post->getNo()}}b">&nbsp b.</label></span>
                  <span class="sec3-q4-c1-tight"><input type="radio" name="{{$post->getNo()}}" id="{{$post->getNo()}}c" value="c" {{ $Q5S3Q4->checkValue($post->getNo(),"c") }} onclick=" myFunction(this)" /><label for="{{$post->getNo()}}c">&nbsp c.</label></span>
                  </p>
                  @endforeach
                </div>
              </div>
