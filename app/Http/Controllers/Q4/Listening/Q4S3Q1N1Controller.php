...
class Q4S3Q1N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089040101");
        }
        ...
        return view('Q4\Listening\Q4S3Q1N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        $section3Question1Id = $userID.".Q4S3Q1";
        $questionDataLoad = Session::get($section3Question1Id);
        Session::put($userID.".Q4S3Q1Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q4S3Q1_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q4S3Q1Score_anchor", 10.5 / 35 * 60 / 8);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',4)->where('section',3)->where('question',1)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',4)->where('section',3)->where('question',1)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_43_01',
        ...
        Q4Section3Question1::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q4Section3Question1::raw("past_testee_number + 1"),
            "correct_testee_number" => Q4Section3Question1::raw("correct_testee_number + 1")
        ]);
        Q4Section3Question1::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q4Section3Question1::raw("past_testee_number + 1")
        ]);
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q4Section3Question1::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();
        ...
        foreach ($results as $rec) {
            ...
            $question = new Q4S3Q1($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
            ...
            // $question = new Q4S3Q1($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
            //             $rec->past_testee_number, $rec->correct_testee_number, $rec->question,
            //             $rec->pattern, 'はこの　おちゃを　かいぎの　へやに　もって　いく。', 'はこの　おちゃを　かいぎの　へやに　もって　いく。', 'はこの　おちゃを　かいぎの　へやに　もって　いく。', 'はこの　おちゃを　かいぎの　へやに　もって　いく。',
            //             $rec->illustration, $rec->listening, $rec->correct_answer, $rec->anchor, $rec->silence, $rec->new_question);
        ...
        while(true){
            ...
            // テキスト問題が３問、重複なしならループから抜ける
            // if($textOnly == 3 && !($this->hasDupe($array)) && !($this->hasDupe($group1Array)) && !($this->hasDupe($group2Array))){
            //     break;
            // }
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(4, 3, 1, $question->getQid(), $no));
            $question->setNo($no++);
        }
