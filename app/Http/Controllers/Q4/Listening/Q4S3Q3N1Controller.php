...
class Q4S3Q3N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089040001"); // 2020/12/10 TOKYO LEVEL4 0001
        }
        ...
        return view('Q4\Listening\Q4S3Q3N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        foreach ($questionDataLoad as $question) {
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',4)->where('section',3)->where('question',3)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',4)->where('section',3)->where('question',3)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_43_03',
        ...
        Q4Section3Question3::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q4Section3Question3::raw("past_testee_number + 1"),
            "correct_testee_number" => Q4Section3Question3::raw("correct_testee_number + 1")
        ]);
        Q4Section3Question3::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q4Section3Question3::raw("past_testee_number + 1")
        ]);
        // $perfectScore = Config::get('constants.Q4S3Q3.perfectScore');
        $rate = round($scoring * 100 / 5);
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            's3_q3_correct' => $scoring,
            's3_q3_question' => 5,
            's3_q3_perfect_score' => 6.5 / 35 * 60,
            's3_q3_anchor_pass' => $anchorFlagResult,
            's3_q3_rate' => $rate]);
        // $scoreQ5S321 = Session::get('Q4S3Q3Score');
        // $request->session()->flush();
        $scoreQ4S3Q3 = $scoring;
        // Session::put('Q4S3Q3Score',$scoreQ4S3Q3);
        Session::put($userID.'.Q4S3Q3Score', $scoreQ4S3Q3);
        Session::put('idTester',$userID);

        return Redirect::to(url('/Q4ListeningQ4'));
    }
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q4Section3Question3::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q4S3Q3($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->sentence, $rec->vocabulary, $rec->question,
        ...
        foreach($questionArray as $question){
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(4, 3, 3, $question->getQid(), $no));
            $question->setNo($no++);
