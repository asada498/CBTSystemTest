...
use App\QuestionClass\Q4\Vocabulary\Q4S1Q1;
...
class Q4S1Q1Controller extends Controller
{
    public function showQuestion(Request $request)
    {
        $currentId = Session::get('idTester');
        $section1Question1Id = $currentId.".Q4S1Q1";
        ...
        // $data->withPath('Q4VocabularyQ1');
        // return view('Q4\Vocabulary\paginationQ4S1Q1',compact('data'));
        // dd("LOL");
        return view('Q4\Vocabulary\paginationQ4S1Q1', compact('data')); //
    ...
    public function getResultToCalculate(Request $request)
    {
        ...
        foreach(Session::get($userID) as $key => $obj)
        {
            ...
            {
                if ($key !== 'Q4S1Q1Score' && $key !== 'Q4S1Q1Score_anchor' )
                {
        ...
        return Redirect::to(url('/Q4VocabularyQ2'));
    }
    ...
    function fetchData(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return view('Q4\Vocabulary\paginationDataQ4S1Q1', compact('data'))->render(); //
        }
    }
    ...
    public function saveChoiceRequestPost(Request $request)
    {
        ...
        $valueSession = $currentId.".Q4S1Q1_".$questionNumber;
    ...
    {
        $currentId = Session::get('idTester');
        $section1Question1Choice = $currentId.".Q4S1Q1_".$questionNumber;
    ...
    function showDataBase()
    {
        ...
        $results = Q4Section1Question1::where('usable', 1)->whereBetween('correct_answer_rate',[0.2,0.8])->get();
        foreach ($results as $user) {
            ...
            $group2 = $user->class_kanji_reading;
            $anchor = $user->anchor;
            $kanji = $user->kanji;
            $idQuestion = $user->id;
            ...
            else 
            {
                ...
                case "100":
                case "030":
                    array_push($affixAdvSuruId, $idQuestion); //9 Affix 1 question
                    break;
            ...
            switch ($group2) {
                ...
                case "102":
                    array_push($type102Id, $idQuestion);
                    break;

                case "103":

                    array_push($type103Id, $idQuestion);
                    break;
    ...
    function getRandomQuestionId($nounId,$verbId,$iAdjectiveId,$naAdjectiveId,$affixAdvSuruId,$anchorId,$type101Id,$type102Id,$type103Id,$type104Id,$type301Id,$type302Id,$type303Id,$newQuestionId,$valueArray)
    {
        // dd($newQuestionId);
        // dd($nounId,$verbId,$iAdjectiveId,$naAdjectiveId,$affixAdvSuruId,$anchorId,$type101Id,$type102Id,$type103Id,$type104Id,$type301Id,$type302Id,$type303Id,$newQuestionId);
        $counterLoop = 0;
        $result = [];
        while ($counterLoop == 0)
        {
        ...
        if (!empty($anchorId))
        {
            ...
            foreach ($valueArray as $val) {
                if ($val->getId() === $anchorId1) {
                    $valueAnchorPartOfSpeech = $val->getGroup1();           
                    $valueAnchorGroup = $val->getGroup2();
                    if ($valueAnchorGroup == "080" || $valueAnchorGroup == "100" || $valueAnchorGroup == "030" ){
                        $valueAnchorGroup = "080100030";
                    }
                }
            }
        }
        ...
        if (!empty($newQuestionId))
        {
            ...
            foreach ($valueArray as $val) {
                if ($val->getId() === $newQuestionId1) {
                    ...
                    if ($valueNewQuestionGroup == "080" || $valueNewQuestionGroup == "100" || $valueNewQuestionGroup == "030" ){
                        $valueNewQuestionGroup = "080100030";
                    }
        ...
        $partOfSpeechArray = ["010","010","040","040","050","060","080100030"];
        $groupArray = ["101","102","103","104","301","302","303"];
            ...
            if (array_search($valueNewQuestionGroup, $groupArray) == true)
            {
                ...
                for ($x = 0; $x < $lengthPairArray; $x++) {
                    ...
                    switch ($groupId) {
                        ...
                        case "102":
                            $groupArrayChoice = $type102Id;
                            break;
        
                        case "103":
                            $groupArrayChoice = $type103Id;
                            break;
                        case "104":
                    ...
                    switch ($partOfSpeechId){
                        ...
                        case "080100030":
                            $partOfSpeechChoice = $affixAdvSuruId;
                            break;
                    }
