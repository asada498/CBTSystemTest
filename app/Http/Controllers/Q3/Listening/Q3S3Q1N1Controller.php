...
use App\QuestionClass\Q3\Listening\Q3S3Q1;
use App\QuestionDatabase\Q3\Listening\Q3Section3Question1;


class Q3S3Q1N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089030101");
        }
        ...
        return view('Q3\Listening\Q3S3Q1N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        $section3Question1Id = $userID.".Q3S3Q1";
        $questionDataLoad = Session::get($section3Question1Id);
        Session::put($userID.".Q3S3Q1Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q3S3Q1_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q3S3Q1Score_anchor", 11 / 40 * 60 / 6);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',3)->where('section',3)->where('question',1)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',3)->where('section',3)->where('question',1)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_33_01',
        ...
        Q3Section3Question1::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q3Section3Question1::raw("past_testee_number + 1"),
            "correct_testee_number" => Q3Section3Question1::raw("correct_testee_number + 1")
        ]);
        Q3Section3Question1::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q3Section3Question1::raw("past_testee_number + 1")
        ]);
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q3Section3Question1::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q3S3Q1($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
                        $rec->past_testee_number, $rec->correct_testee_number, $rec->question,
                        $rec->pattern, $rec->choice_a, $rec->choice_b, $rec->choice_c, $rec->choice_d,
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(3, 3, 1, $question->getQid(), $no));
            $question->setNo($no++);
        }
