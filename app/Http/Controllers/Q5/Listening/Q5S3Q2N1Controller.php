...
class Q5S3Q2N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089050101"); // 2020/12/10 TOKYO LEVEL5 0101
        }
        $currentId = Session::get('idTester');
        $section3Question2Id = $currentId.".Q5S3Q2";
        ...
        return view('Q5\Listening\Q5S3Q2N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        if (!(Session::has('idTester')))
                {
                    $userIDTemp = "Q20121089050001";
                    Session::put('idTester',$userIDTemp);        
                }
        $userID = Session::get('idTester');
        $section3Question2Id = $userID.".Q5S3Q2";
        $questionDataLoad = Session::get($section3Question2Id);
        Session::put($userID.".Q5S3Q2Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q5S3Q2_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q5S3Q2Score_anchor", 9 / 30 * 60 / 6);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',5)->where('section',3)->where('question',2)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',5)->where('section',3)->where('question',2)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_53_02',
        ...
        Q5Section3Question2::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q5Section3Question2::raw("past_testee_number + 1"),
            "correct_testee_number" => Q5Section3Question2::raw("correct_testee_number + 1")
        ]);
        Q5Section3Question2::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q5Section3Question2::raw("past_testee_number + 1")
        ]);
        //$perfectScore = Config::get('constants.Q5S3Q2.perfectScore');
        $rate = round($scoring * 100 / 6);
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            's3_q2_correct' => $scoring,
            's3_q2_question' => 6,
            's3_q2_perfect_score' => 9 / 30 * 60,
            's3_q2_anchor_pass' => $anchorFlagResult,
            's3_q2_rate' => $rate]);
        // $scoreQ5S321 = Session::get('Q5S3Q2Score');
        // $request->session()->flush();
        $scoreQ5S3Q2 = $scoring;
        // Session::put('Q5S3Q2Score',$scoreQ5S3Q2);
        Session::put($userID.'.Q5S3Q2Score', $scoreQ5S3Q2);
        Session::put('idTester',$userID);
       return Redirect::to(url('/Q5ListeningQ3N1'));
    }
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q5Section3Question2::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q5S3Q2($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
            ...
            if ($rec->new_question == '1'){
                ...
                case $this->GROUP_CODE_WHEATHER: // 天気
                    ...
                    break;
                default:
                    throw new Exception('GROUP3 CODE ERROR');
                }
            }
        }
        ...
        while(true){
            ...
            // テキスト問題が４問、設問の重複なしの場合ループから抜ける
            if($textOnly == 4 && !($this->hasDupe($array))){
                break;
            }
            $questionArray = [];
        }
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(5, 3, 2, $question->getQid(), $no));
            $question->setNo($no++);
        }
    ...
    // function getRandomQuestionId ($nounId,$katakanaId,$verbId,$iOrNaAdjectiveId,$affixId,$numberId)
    // {
    //     $arrayNounId = array_rand($nounId, 2);
    //     $nounId1 = $nounId[$arrayNounId[0]];
    //     $nounId2 = $nounId[$arrayNounId[1]];

    //     $arrayKatakanaId = array_rand($katakanaId, 2);
    //     $katakanaId1 = $katakanaId[$arrayKatakanaId[0]];
    //     $katakanaId2 = $katakanaId[$arrayKatakanaId[1]];
    ...
    //     $arrayAffixId = array_rand($affixId, 1);
    //     $affixId1 = $affixId[$arrayAffixId]; 

    //     $arrayNumberId = array_rand($numberId, 1);
    //     $numberId1 = $numberId[$arrayNumberId];

    //     return [$nounId1,$nounId2,$katakanaId1,$katakanaId2,$verbId1,$iOrNaAdjectiveId1,$affixId1,$numberId1];
    // }
}