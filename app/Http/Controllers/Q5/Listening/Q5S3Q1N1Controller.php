...
class Q5S3Q1N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089050101");
        }
        ...
        return view('Q5\Listening\Q5S3Q1N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        $section3Question1Id = $userID.".Q5S3Q1";
        $questionDataLoad = Session::get($section3Question1Id);
        Session::put($userID.".Q5S3Q1Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q5S3Q1_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                if ($anchorFlag == 1){
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q5S3Q1Score_anchor", 9 / 30 * 60 / 7);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',5)->where('section',3)->where('question',1)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',5)->where('section',3)->where('question',1)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_53_01',
        ...
        Q5Section3Question1::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q5Section3Question1::raw("past_testee_number + 1"),
            "correct_testee_number" => Q5Section3Question1::raw("correct_testee_number + 1")
        ]);
        Q5Section3Question1::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q5Section3Question1::raw("past_testee_number + 1")
        ]);
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q5Section3Question1::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q5S3Q1($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
        ...
        while(true){
            ...
            // テキスト問題が３問、設問の重複なしの場合ループから抜ける
            if($textOnly == 3 && !($this->hasDupe($array))){
                break;
            }
            $questionArray = [];
        }
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(5, 3, 1, $question->getQid(), $no));
            $question->setNo($no++);
        }
