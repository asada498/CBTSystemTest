...
use App\QuestionClass\Q1\Listening\Q1S3Q5;
use App\QuestionDatabase\Q1\Listening\Q1Section3Question5;

class Q1S3Q5Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        $section3Question5Id = $currentId.".Q1S3Q5";
        ...
        return view('Q1\Listening\Q1S3Q5', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate(Request $request)
    {
        ...
        foreach ($questionDataLoad as $question) {
            ...
            if (AnswerRecord::where('examinee_number', $userID)->where('level', 1)->where('section', 3)->where('question', 5)->where('number', $questionId)->exists()) {
                AnswerRecord::where('examinee_number', $userID)->where('level', 1)->where('section', 3)->where('question', 5)->where('number', $questionId)->update(
                    [
                        'question_type' => $codeQuestion,
                        'question_table_name' => 'q_13_05',
        ...
        Q1Section3Question5::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q1Section3Question5::raw("past_testee_number + 1"),
            "correct_testee_number" => Q1Section3Question5::raw("correct_testee_number + 1")
        ]);
        Q1Section3Question5::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q1Section3Question5::raw("past_testee_number + 1")
        ]);
        ...
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            ...
            's3_q5_question' => 4,
            's3_q5_perfect_score' => 11.5/55*60,
            's3_q5_anchor_pass' => 0,
            's3_q5_rate' => $rate,
            's3_end_flag' => 1
        ]);
        if (!TestResult::where('examinee_id', substr($userID, 1))->where('level', 1)->exists()) {
            TestResult::insert(
                [
        ...
        {
            if (str_starts_with($key,'Q1S3Q5'))
            {
                if ($key !== 'Q1S3Q5Score' && $key !== 'Q1S3Q5Score_anchor' )
                {
        ...
        $s3Q1Correct = Session::get($userID.".Q1S3Q1Score");
        $s3Q2Correct = Session::get($userID.".Q1S3Q2Score");
        $s3Q3Correct = Session::get($userID.".Q1S3Q3Score");
        $s3Q4Correct = Session::get($userID.".Q1S3Q4Score");
        $s3Q5Correct = $scoring;
        $section3Total = $s3Q1Correct *11/55*60/6 + $s3Q2Correct *12/55*60/7 + $s3Q3Correct *12/55*60/6 +
                         $s3Q4Correct *8.5/55*60/14 + $s3Q5Correct *11.5/55*60/4;
        ...
        $anchorScoreQ1S3Q1 = Session::get( $userID.'.Q1S3Q1Score_anchor');
        $anchorScoreQ1S3Q2 = Session::get( $userID.'.Q1S3Q2Score_anchor');
        $anchorScoreQ1S3Q4 = Session::get( $userID.'.Q1S3Q4Score_anchor');
        $currentAnchorScore = $anchorScoreQ1S1Q1 + 
                              $anchorScoreQ1S1Q2 + 
                              $anchorScoreQ1S1Q5 + 
                              $anchorScoreQ1S3Q1 + 
                              $anchorScoreQ1S3Q2 + 
        ...
        $scoreQ1S3Q5 = $scoring;
        Session::put($userID . '.Q1S3Q5Score', $scoreQ1S3Q5);
        ...
        $section1And2Total = $query->s1_q1_correct *4/40*60/6 +
                             $query->s1_q2_correct *5/40*60/7 +
                             $query->s1_q3_correct *6/40*60/6 +
                             $query->s1_q4_correct *6.5/40*60/6 +
                             $query->s1_q5_correct *5.5/40*60/10 +
                             $query->s1_q6_correct *6/40*60/5 +
                             $query->s1_q7_correct *7/40*60/5 +
                             $query->s1_q8_correct *10/70*60/4 +
                             $query->s1_q9_correct *18/70*60/9 +
                             $query->s1_q10_correct *12/70*60/4 +
                             $query->s1_q11_correct *11/70*60/3 +
                             $query->s1_q12_correct *12/70*60/4 +
                             $query->s1_q13_correct *7/70*60/2;
        $section3Total = $query->s3_q1_correct *11/55*60/6 +
                         $query->s3_q2_correct *12/55*60/7 +
                         $query->s3_q3_correct *12/55*60/6 +
                         $query->s3_q4_correct *8.5/55*60/14 +
                         $query->s3_q5_correct *11.5/55*60/4;
        ...
        return Redirect::to(url('/Gradehomepage1'));
    }
    ...
    {       
        ...
        $valueSession = $currentId.".Q1S3Q5_".$questionNumber;
    ...
    {
        $currentId = Session::get('idTester');
        $section3Question5Choice = $currentId.".Q1S3Q5_".$questionNumber;
    ...
    function showDataBase()
    {
        while(true){
            // グループを取得する
            $groupMap = [];
            $results = DB::select("select distinct group1 from q_13_05 where usable = 1 and correct_answer_rate between 0.2 and 0.8");
            ...
            $flag401 = false;
            // 新問の取得
            $results = DB::select("select id, group1 from q_13_05 where new_question = 1 and usable = 1 and correct_answer_rate between 0.2 and 0.8 limit 1");
            if($results != null){
            ...
            //$results = Q1Section3Question5::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();
            $results = Q1Section3Question5::where('usable', '1')->get();
            foreach ($results as $rec) {
                $question = new Q1S3Q5($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
            ...
            if($id3 == 0){
                ...
                $sameArray = [];
                $results = DB::select("SELECT same_passage, count(*) as cnt FROM q_13_05 where usable = 1 and correct_answer_rate between 0.2 and 0.8 and same_passage <> 0 group by same_passage having count(*) = 2");
                foreach ($results as $rec) {
                    array_push($sameArray, $rec->same_passage);
                }
                $same = $sameArray[array_rand($sameArray, 1)];
                $param = ['same' => $same];
                $results = DB::select("SELECT id FROM q_13_05 where same_passage = :same order by qid", $param);
                $id3 = $results[0]->id;
                $id4 = $results[1]->id;
            }else{
            ...
            $count401 = 0;
            if($questionMap[$id1]->group1 == $this->GROUP_CODE_INTEG)
                $count401++;
            if($questionMap[$id2]->group1 == $this->GROUP_CODE_INTEG)
                $count401++;
            if($questionMap[$id3]->group1 == $this->GROUP_CODE_INTEG)
                $count401++;
            if($questionMap[$id4]->group1 == $this->GROUP_CODE_INTEG)
                $count401++;

            $array = [];
            array_push($array, $id1);
            array_push($array, $id2);
            array_push($array, $id3);
            array_push($array, $id4);
            // 統合２件で、IDの重複がなければループから抜ける
            if(!($this->hasDupe($array)) && $count401 == 2){
                break;
            }
        }
        ...
        foreach($questionArray as $question){
            ...
            case 2:
            case 3:
                $question->setBanFile($this->searchBanFile(1, 3, 5, $question->getQid(), $no));
            }
            $question->setNo($no++);
        }
