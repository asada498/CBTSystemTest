...
class Q1S3Q2N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        $section3Question2Id = $currentId.".Q1S3Q2";
        $questionData = $this->showDataBase();
        Session::put($section3Question2Id, $questionData);
        
        return view('Q1\Listening\Q1S3Q2N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        $section3Question2Id = $userID.".Q1S3Q2";
        $questionDataLoad = Session::get($section3Question2Id);
        Session::put($userID.".Q1S3Q2Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q1S3Q2_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q1S3Q2Score_anchor", 12/55*60/7);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',1)->where('section',3)->where('question',2)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',1)->where('section',3)->where('question',2)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_13_02',
        ...
        Q1Section3Question2::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q1Section3Question2::raw("past_testee_number + 1"),
            "correct_testee_number" => Q1Section3Question2::raw("correct_testee_number + 1")
        ]);
        Q1Section3Question2::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q1Section3Question2::raw("past_testee_number + 1")
        ]);
        ...
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            ...
            's3_q2_question' => 7,
            's3_q2_perfect_score' => 12/55*60,
            's3_q2_anchor_pass' => $anchorFlagResult,
            's3_q2_rate' => $rate]);
        $scoreQ1S3Q2 = $scoring;
        Session::put($userID.'.Q1S3Q2Score', $scoreQ1S3Q2);
        Session::put('idTester',$userID);
       return Redirect::to(url('/Q1ListeningQ3'));
    }
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q1Section3Question2::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q1S3Q2($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(1, 3, 2, $question->getQid(), $no));
            $question->setNo($no++);
        }
