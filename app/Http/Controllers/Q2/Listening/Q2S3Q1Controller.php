...
use App\QuestionClass\Q2\Listening\Q2S3Q1;
use App\QuestionDatabase\Q2\Listening\Q2Section3Question1;

class Q2S3Q1Controller extends Controller
{
    ...
    //private $GROUP_CODE_THOUGHTS = '090'; // 内容：考え (考えていること、心配していること等)
    //private $GROUP_CODE_STORY    = '990'; // 内容：話 (約束、アドバイス含む)
    //private $GROUP_CODE_CONTENTS = '110'; // 内容：出来事
    ...
    //private $GROUP_CODE_ORGANISM = '119'; // 人物・動物・植物
    ...
    public function showQuestion (){
        ...
        $section3Question1Id = $currentId.".Q2S3Q1";
        ...
        return view('Q2\Listening\Q2S3Q1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        $section3Question1Id = $userID.".Q2S3Q1";
        $questionDataLoad = Session::get($section3Question1Id);
        Session::put($userID.".Q2S3Q1Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q2S3Q1_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q2S3Q1Score_anchor", 12/5);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',2)->where('section',3)->where('question',1)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',2)->where('section',3)->where('question',1)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_23_01',
        ...
        Q2Section3Question1::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q2Section3Question1::raw("past_testee_number + 1"),
            "correct_testee_number" => Q2Section3Question1::raw("correct_testee_number + 1")
        ]);
        Q2Section3Question1::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q2Section3Question1::raw("past_testee_number + 1")
        ]);
        ...
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            ...
            's3_q1_question' => 5,
            's3_q1_perfect_score' => 12,
            's3_q1_anchor_pass' => $anchorFlagResult,
            's3_q1_rate' => $rate]);
        // $scoreQ5S3Q1 = Session::get('Q5S3Q1Score');
        // $request->session()->flush();
        // Session::put('Q5S3Q1Score',$scoreQ5S3Q1);
        Session::put($userID.'.Q2S3Q1Score', $scoring);
        Session::put('idTester',$userID);
        return Redirect::to(url('/Q2ListeningQ2N1'));
    }
    ...
    {       
        ...
        $valueSession = $currentId.".Q2S3Q1_".$questionNumber;
    ...
    {
        $currentId = Session::get('idTester');
        $section3Question1Choice = $currentId.".Q2S3Q1_".$questionNumber;
    ...
    function showDataBase()
    {
        ...
        // $thoughtsIdArray = [];  // 内容：考え (考えていること、心配していること等)
        // $storyIdArray = [];     // 内容：話 (約束、アドバイス含む)
        // $contentsIdArray = [];  // 内容：出来事
        $purposeIdArray = [];   // 目的
        ...
        $methodIdArray = [];    // 方法・手段
        $weatherIdArray = [];   // 天気
        // $organismIdArray = [];  // 人物・動物・植物
        $anchorIdArray = [];    // アンカー
        $newQuestionArray = []; // 新問
        $groupMap = [];

        // 問題を取得する 
        $results = Q2Section3Question1::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q2S3Q1($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
            ...
            if ($rec->new_question == '1'){
                ...
                case $this->GROUP_CODE_NUMBER: // 番号 (電話番号、ページなど)
                    ...
                    break;
                // case $this->GROUP_CODE_STUFF: // 物 (食べ物、服など)
                //     array_push($stuffIdArray, $idQuestion);
                //     break;
                ...
                case $this->GROUP_CODE_SINGULAR: // 選択・単数
                    ...
                    break;
                // case $this->GROUP_CODE_THOUGHTS: // 内容：考え (考えていること、心配していること等)
                //     array_push($thoughtsIdArray, $idQuestion);
                //     break;
                // case $this->GROUP_CODE_STORY: // 内容：話 (約束、アドバイス含む)
                //     array_push($storyIdArray, $idQuestion);
                //     break;
                // case $this->GROUP_CODE_CONTENTS: // 内容：出来事
                //     array_push($contentsIdArray, $idQuestion);
                //     break;
                ...
                case $this->GROUP_CODE_WHEATHER: // 天気
                    ...
                    break;
                // case $this->GROUP_CODE_ORGANISM: // 人物・動物・植物
                //     array_push($organismIdArray, $idQuestion);
                //     break;
                default:
        ...
        while(true){
            ...
            for($i = 0; $i < $count; $i++){
                ...
                case $this->GROUP_CODE_NUMBER: // 番号 (電話番号、ページなど)
                    ...
                    break;
                // case $this->GROUP_CODE_STUFF: // 物 (食べ物、服など)
                //     array_push($questionArray, $questionMap[$stuffIdArray[array_rand($stuffIdArray, 1)]]);
                //     break;
                case $this->GROUP_CODE_ORDER: // 順番
                ...
                case $this->GROUP_CODE_SINGULAR: // 選択・単数
                    ...
                    break;
                // case $this->GROUP_CODE_THOUGHTS: // 内容：考え (考えていること、心配していること等)
                //     array_push($questionArray, $questionMap[$thoughtsIdArray[array_rand($thoughtsIdArray, 1)]]);
                //     break;
                // case $this->GROUP_CODE_STORY: // 内容：話 (約束、アドバイス含む)
                //     array_push($questionArray, $questionMap[$storyIdArray[array_rand($storyIdArray, 1)]]);
                //     break;
                // case $this->GROUP_CODE_CONTENTS: // 内容：出来事
                //     array_push($questionArray, $questionMap[$contentsIdArray[array_rand($contentsIdArray, 1)]]);
                //     break;
                case $this->GROUP_CODE_PURPOSE: // 目的
                ...
                case $this->GROUP_CODE_WHEATHER: // 天気
                    ...
                    break;
                // case $this->GROUP_CODE_ORGANISM: // 人物・動物・植物
                //     array_push($questionArray, $questionMap[$organismIdArray[array_rand($organismIdArray, 1)]]);
                //     break;
                default:
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(2, 3, 1, $question->getQid(), $no));
            $question->setNo($no++);
        }
