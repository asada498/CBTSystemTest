...
class Q2S3Q2N1Controller extends Controller
{
    ...
    public function showQuestion (){
        ...
        if (!(Session::has('idTester'))) {
            Session::put('idTester', "Q20121089020101"); // 2020/12/10 TOKYO LEVEL4 0101
        }
        $currentId = Session::get('idTester');
        $section3Question2Id = $currentId.".Q2S3Q2";
        ...
        return view('Q2\Listening\Q2S3Q2N1', ['data' => $questionData]);
    }
    ...
    public function getResultToCalculate (Request $request){
        ...
        if (!(Session::has('idTester')))
                {
                    $userIDTemp = "Q20121089030001";
                    Session::put('idTester',$userIDTemp);        
                }
        $userID = Session::get('idTester');
        $section3Question2Id = $userID.".Q2S3Q2";
        $questionDataLoad = Session::get($section3Question2Id);
        Session::put($userID.".Q2S3Q2Score_anchor", 0);

        foreach ($questionDataLoad as $question) {
            $questionId = $question->getNo();
            $currentQuestion = $userID.'.Q2S3Q2_'.$questionId;
            ...
            if ($question->getCorrectAnswer() == $userAnswer)
            {
                ...
                {
                    $anchorFlagResult = 1;
                    Session::put($userID.".Q2S3Q2Score_anchor", 13.8/6);
                }
                $scoring++;
            ...
            if (AnswerRecord::where('examinee_number',$userID)->where('level',2)->where('section',3)->where('question',2)->where('number',$questionId)->exists())
            {
                AnswerRecord::where('examinee_number',$userID)->where('level',2)->where('section',3)->where('question',2)->where('number',$questionId)->update(
                    [
                     'question_type'=>$codeQuestion,
                     'question_table_name'=>'q_23_02',
        ...
        Q2Section3Question2::whereIn('id', $correctAnswer)->update([
            "past_testee_number" => Q2Section3Question2::raw("past_testee_number + 1"),
            "correct_testee_number" => Q2Section3Question2::raw("correct_testee_number + 1")
        ]);
        Q2Section3Question2::whereIn('id', $incorrectAnswer)->update([
            "past_testee_number" => Q2Section3Question2::raw("past_testee_number + 1")
        ]);
        ...
        ScoreSummary::where('examinee_number', substr($userID, 1))->update([
            ...
            's3_q2_question' => 6,
            's3_q2_perfect_score' => 13.8,
            's3_q2_anchor_pass' => $anchorFlagResult,
            's3_q2_rate' => $rate]);
        // $scoreQ4S321 = Session::get('Q2S3Q2Score');
        // $request->session()->flush();
        $scoreQ2S3Q2 = $scoring;
        // Session::put('Q4S3Q2Score',$scoreQ2S3Q2);
        Session::put($userID.'.Q2S3Q2Score', $scoreQ2S3Q2);
        Session::put('idTester',$userID);
       return Redirect::to(url('/Q2ListeningQ3'));
    }
    ...
    function showDataBase()
    {
        ...
        // 問題を取得する 
        $results = Q2Section3Question2::where('usable', '1')->whereBetween('correct_answer_rate', [0.20, 0.80])->get();

        foreach ($results as $rec) {

            $question = new Q2S3Q2($rec->id, $rec->qid, $rec->group1, $rec->group2, $rec->group3,
        ...
        foreach($questionArray as $question){
            ...
            // XX番の音声ファイルをセット
            $question->setBanFile($this->searchBanFile(2, 3, 2, $question->getQid(), $no));
            $question->setNo($no++);
        }
